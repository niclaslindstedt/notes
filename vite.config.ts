...
// Both wrappers — the native WebView shell (`native/`) and the Tauri desktop
// shell (`tauri/`) — embed a compiled copy of this app and load it from local
// files rather than over HTTP. The native build sets `VITE_TARGET=native`;
// the desktop shell's site build (`tauri/scripts/bundle-web.mjs`) sets
// `VITE_SHELL_BUILD=on`, the one variable every app in the fleet uses for
// that build. Either flips the base to relative (`./`) so the origin-absolute
// `/assets/...` URLs become `./assets/...` and resolve under a private-scheme
// origin, and disables the service worker below (offline is already
// guaranteed by the local bundle; updates ride app releases).
const isEmbedded =
  process.env.VITE_TARGET === "native" || process.env.VITE_SHELL_BUILD === "on";
...
// deploy and the desktop app are the project's own deploys and always
  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        ...
        // The 1024 maskable icon is the desktop shell's packaging source, not
        // a web asset — 21 kB no browser install has a use for.
        globIgnores: ["**/*.map", "**/maskable-icon-1024x1024.png"],
