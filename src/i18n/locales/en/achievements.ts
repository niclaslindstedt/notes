...
const achievements = {
  ...
  catalog: {
    ...
    stylist: {
      ...
      learnMore:
        "The formatting button at the top right of a note opens a toolbar above the text — headings, bold, italic, strikethrough, inline code, bullet and numbered lists, quotes, code blocks, indent and outdent, links, images and files, and dividers, one button each. It writes ordinary Markdown, so anything you reach for there you can also just type; every button is a toggle, so pressing a lit one takes the formatting back off. Image/file is the one that reaches past Markdown: it opens your photo or file browser and attaches what you pick at the cursor, the same as pasting a picture in.",
    },
    ...
    countTheLines: {
      ...
      learnMore:
        "Settings → Editor numbers every line down the left edge, the way a code editor does. The gutter is also the way into select mode: press a number and the mode opens with that line taken, or drag down the numbers to take a run — ready to cut, replace, or restyle.",
    },
    freehand: {
      name: "Freehand",
