...
export type FloatingRect = {
  ...
  // (a long option label) can otherwise balloon the panel off-screen in
  // a narrow column. Consumers apply this as `max-width` so the content
...
// `min`: at least `minPx`, grows to trigger width if larger (the dropdown).
...
function compute(
  ...
  const width =
    placement.width.kind === "min"
      ? Math.max(placement.width.minPx, rect.width)
      : Math.min(placement.width.maxPx, win.innerWidth - 2 * margin);
  ...
  // there's more room above. The threshold matches the
  // listbox-with-a-handful-of-items case the dropdown was designed for:
  // a few option rows. Below that, an upward flip is the difference
  // between "showing 1 item with a scrollbar pinned to the
  // visible-bottom margin" and "showing the whole list growing toward
  // the top of the screen". Users working with a control near the
  // bottom of a long settings modal hit the former constantly, which is
  // the bug this flip fixes.
  ...
  if (verticalPlacement === "above") {
    ...
    // Viewport-coord floats (`position: fixed` anchored to the layout
    // viewport) need clamping into the visible region — they don't
    // scroll with the page, so iOS's visual-viewport shift for the
    // keyboard could leave them above the visible area where the user
    // can never reach them. Document-coord floats opt out: they scroll
    // with the page and ride along with the trigger row.
    if (!document) {
