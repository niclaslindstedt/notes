...
| **Swipe down to hide the keyboard** / **pull down to dismiss the keyboard** / **the floating bar over the keyboard** / **keyboard accessory bar** | `useSwipeDownDismiss` (`src/ui/hooks/useSwipeDownDismiss.ts`), wired into the editors in `MarkdownEditor.tsx` / `NoteEditor.tsx`; the iOS accessory bar is hidden in the native wrapper via `hideKeyboardAccessoryView` (`native/src/WebViewHost.tsx`); the `swipeAway` achievement. [→](overview.md#swipe-down-to-dismiss-keyboard) |
