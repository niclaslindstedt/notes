...
| **Route** / **router** / **browser back** / **back button** / **back and forward** / **go back to the previous note** / **history entry** | `Route`, `useRoute`, `go` / `replace` / `backTo`, `noteRoute` (`src/app/use-route.ts`); projected to `editingId` / `readingId` / `view` in `src/app/App.tsx`; the `retrace` achievement. [→](overview.md#route--browser-back--forward) |
...
| **Button island** / **action bar** / **action panel** / **New note / New folder / Show all / Archive + Undo / Redo / Search buttons** (sidebar) | the bordered `BarButton` block (two rows split by a divider) pinned above the footer, its own `SideMenuActionBar` component (`src/ui/SideMenuActionBar.tsx`) rendered by `SideMenu`. [→](overview.md#folders-in-the-side-menu) |
