...
| **Bare URL** / **autolink** / **raw link** (no `[…](…)`) | `matchAutolink` in `parseInline` (`src/domain/markdown.ts`); turns a typed `http(s)://` / `www.` URL into a `link` node. [→](overview.md#markdown-parser) |
