...
| **Styling toolbar** / **formatting toolbar** / **format bar** / **the button top right** / **bold / italic / heading buttons** / **bullet point children** (the indent buttons) | `FormatToolbar`, `FormatToolbarButton` (`src/ui/FormatToolbar.tsx`); glyphs `src/ui/format-glyphs.tsx`; the pure engine `applyFormat`, `lineFormatAt` / `lineFormatOf`, `FormatAction`, `LineFormat` (`src/domain/markdown-format.ts`); `format` on `MarkdownEditorHandle` + `placeRange` (`src/ui/contenteditable-caret.ts`); `notes/format-toolbar` (localStorage); the `format-toolbar-in` animation (`src/styles/theme.css`); the **Stylist** achievement. [→](overview.md#styling-toolbar) |
