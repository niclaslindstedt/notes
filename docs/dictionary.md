...
| **Undo** / **redo** / **undo timeline** / **history** / **per-note undo** / **undo per note** / **sentence-level undo** / **undo sentence by sentence** | `useUndoRedo`, `UndoRedo`, `UNDO_HISTORY_LIMIT`, `DOC_SCOPE` (the structural-changes timeline), `mergeDocSnapshot` (surgical structural apply), `nextEditRun` (the type→erase→retype run counter) (`src/app/use-undo-redo.ts`); `sentenceBoundaryCount` (`src/domain/sentence.ts`, the per-sentence merge-key granularity); `useUndoRedoShortcuts` (`src/ui/hooks/useUndoRedoShortcuts.ts`). [→](overview.md#undo--redo) |
