...
| **Encryption migration** / **encryption conversion** / **background queue** / **green lock** / **watch it encrypt** / **watch it decrypt** / **Fort Knox** | `runEncryptionMigration` (`src/storage/encryption-migration.ts`), `useEncryptionMigration` + `EncryptionConversionState` (`src/app/use-encryption-migration.ts`); `migrateNote`/`demigrateNote`/`getEncryptionStatus`/`splitLegacyBlob` (`src/storage/directory-adapter.ts`); `encryptionDisabling`/`finishDisableEncryption` (`src/storage/useStorageBackend.ts`); `LockIcon` (`src/ui/icons.tsx`). [→](overview.md#encryption-migration) |
| **Note index** / **encrypted index** / **fast unlock** / **deferred body** / **lazy body** / **load note on open** / **background warm** | `note-index.ts` (`IndexEntry`, `noteToIndexEntry`, `indexEntryToNote`, `serializeIndex`/`parseIndex`); `INDEX_FILE_NAME` + `readIndexEntries`/`sealWriteIndex`/`fetchNoteBody` (`src/storage/directory-adapter.ts`); `Note.body?`/`Note.preview` (`src/domain/note.ts`); `ensureBody` + the warm loop (`src/app/use-notes.ts`); body mirror in `withLocalCache` (`src/storage/cache/index.ts`). [→](overview.md#encryption) |
...
| **Folder placement** / **folders on top** / **sort folders with notes** / **mixed in** | `Appearance.folderPlacement`, `FolderPlacement`, `FOLDER_PLACEMENTS` (`src/theme/themes.ts`); `mixTopLevel` + the **Sidebar** group in `AppearanceSection`. [→](overview.md#folders-in-the-side-menu) |
| **Sidebar sort** / **sort by name** / **sort by last modified** | `Appearance.noteSortKey`, `NoteSortKey`, `NOTE_SORT_KEYS` (`src/theme/themes.ts`); `sortNotesBy` / `sortFoldersBy` in `SideMenu`. [→](overview.md#folders-in-the-side-menu) |
...
| **Folder section** / **drag a note into a folder** / **swipe / right-click a folder** (overview) | the folder grouping + drop targets in `NoteList`; `OverviewFolderHeader` (swipe / right-click to edit/delete), `FolderRenameRow` (inline rename) (`src/app/App.tsx`). [→](overview.md#folders-in-the-overview) |
