...
| **Header** / **the top bar**                               | the sticky `<header>` each main surface renders (`NoteList`, `NoteEditor`, `ArchivedNoteView`) — app title, and in the editor the per-note action cluster (find / formatting / cut / copy). [→](overview.md#app-shell) |
...
| **Transform tab** / **the transform settings** / **add a transform** / **the rule dialog** / **sample and result** / **mask style** | `TransformSection` (`src/ui/settings/TransformSection.tsx`) — the list; `TransformRuleModal` (`src/ui/settings/TransformRuleModal.tsx`) — the add/edit dialog with the live sample → result pane; `WandIcon` (`src/ui/icons.tsx`) marks the tab. [→](overview.md#transform-settings) |
| **Regex reference** / **regex helper** / **the dropdown of special characters** / **what does `\d` mean** / **insert a capture group** / **regex cheat sheet** | `RegexHelper` (`src/ui/settings/RegexHelper.tsx`) — the dropdown under the pattern field; `REGEX_TOKEN_GROUPS` + `insertRegexToken` (`src/domain/transform.ts`) — the catalog and the pure caret insertion; descriptions live in `settings.transform.token.<id>` (`src/i18n/locales/{en,sv}/settings.ts`). [→](overview.md#transform-settings) |
...
| **Export** / **export button** / **the up arrow** / **export menu** / **export to PDF** / **export to MD** / **save as PDF** / **print a note** / **download a note** | `ExportButton` (`src/ui/export/ExportButton.tsx`); the actions `exportPdf` / `downloadMarkdown` / `exportFileStem` (`src/ui/export/export-note.ts`); `printHtmlDocument` (`src/ui/export/print-document.ts`); `ExportIcon` / `FilePdfIcon` / `FileMarkdownIcon` (`src/ui/icons.tsx`); the **Printing press** / **Takeaway** achievements. [→](overview.md#export)
| **PDF settings** / **PDF renderer** / **print layout** / **page margins** / **paper size** / **code background** / **bullet glyph (PDF)** | `PdfSettings`, `DEFAULT_PDF_SETTINGS`, `coercePdfSettings`, `bulletGlyphAt` (`src/domain/pdf.ts`, re-exported from `src/theme/themes.ts`); `renderPrintDocument` (`src/domain/pdf-render.ts`); `ExportSection` (`src/ui/settings/ExportSection.tsx`); `Appearance.pdf` (`src/theme/useTheme.ts`). [→](overview.md#pdf-settings)
| **Copy button** / **copy glyph** / **copy note** / **copy scope** / **copy with front matter** / **where did the copy button go** | the **Copy to clipboard** row of `ExportButton` (`src/ui/export/ExportButton.tsx`) — there is no separate copy button; `buildCopyText` (`src/ui/copy-note.ts`); `CopyScope` (`src/domain/note.ts`). [→](overview.md#copy-scope) |
| **Cut button** / **the scissors button** / **the X— button** (its artwork until the cut rework) / **cut the selection** / **remove a line** / **kill the rest of the line** / **Ctrl+K** | `CutButton` (`src/ui/CutButton.tsx`); `CutIcon` (`src/ui/icons.tsx`); the pure `cutLine` + `sliceRange` (`src/domain/line-edit.ts`); `writeClipboard` (`src/ui/clipboard.ts`); `cut` on `MarkdownEditorHandle` (`src/ui/MarkdownEditor.tsx`) and on the plain fallback + `runCut` (`src/ui/NoteEditor.tsx`); the **Guillotine** achievement. [→](overview.md#cut-button) |
...
| **Find in note** / **find bar** / **search in the note** / **search bar** / **the magnifier in the note header** / **next / previous match** / **highlight the matches** | `NoteFindBar`, `NoteFindButton` (`src/ui/NoteFindBar.tsx`); the pure engine `findMatches`, `NoteMatch` (`src/domain/note-find.ts`); the `matches` / `activeMatch` props on `MarkdownEditor` / `PlainEditor`; the `<mark>` painting in `markSource` / `LineHighlight` (`src/ui/MarkdownLine.tsx`); the **Pinpoint** achievement. [→](overview.md#find-in-note) |
...
| **Search** / **search modal** / **find a note** / **search across notes** | `SearchModal` (`src/ui/SearchModal.tsx`); engine `buildSearchIndex` / `search` / `segmentMatches` (`src/domain/search.ts`); host `SearchModalHost` (`src/app/modals/SearchModalHost.tsx`); `{ kind: "search" }` on the modal bus; the Search `BarButton` (`SearchIcon`) in `src/ui/SideMenuActionBar.tsx`. [→](overview.md#search) |
