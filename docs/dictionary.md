...
| **Header** / **the top bar**                               | the sticky `<header>` each main surface renders (`NoteList`, `NoteEditor`, `ArchivedNoteView`) — app title, and in the editor the per-note action cluster (find / formatting / delete-line / copy). [→](overview.md#app-shell) |
...
| **Delete-line button** / **the X— button** / **remove a line** / **kill the rest of the line** / **Ctrl+K** | `DeleteLineButton` (`src/ui/DeleteLineButton.tsx`); `DeleteLineIcon` (`src/ui/icons.tsx`); the pure `deleteLine` (`src/domain/line-edit.ts`); `deleteLine` on `MarkdownEditorHandle` (`src/ui/MarkdownEditor.tsx`) and on the plain fallback + `runDeleteLine` (`src/ui/NoteEditor.tsx`); the **Guillotine** achievement. [→](overview.md#delete-line-button) |
