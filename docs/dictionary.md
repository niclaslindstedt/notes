...
| **Select across lines** / **drag to select** / **select all** / **copy a selection** / **selection mapping** | native selection over the single `contenteditable` surface in `MarkdownEditor` (the `onCopy` / `onCut` handlers, the Ctrl/Cmd+A branch of `onKeyDown`); `sourcePointFromDom`, `extractSourceRange` (`src/ui/markdown-selection.ts`). [→](overview.md#selection-mapping) |
