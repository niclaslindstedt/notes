...
| **Code block** / **fenced code** / **the triple backticks** / **hide the fences** / **``` lines** | `fencedRanges`, `hiddenFenceLines`, `hasClosedFence` (`src/domain/markdown.ts`); the `fence` / `code` cases in `RenderedLine` (`src/ui/MarkdownLine.tsx`) and `lineTextClass` (`src/ui/markdown-line-class.ts`); the `hiddenFences` set in `MarkdownEditor` (`src/ui/MarkdownEditor.tsx`). [→](overview.md#code-block) |
