...
`MarkdownEditor`'s paste / drop handlers — and the `attach` verb on its handle,
which the toolbar's picker (`pickFiles`, `src/ui/attachments/pick-files.ts`)
hands its files to — build the attachment
